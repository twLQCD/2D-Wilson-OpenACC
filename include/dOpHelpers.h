...
#pragma acc data present(gauge[0:LX][0:LY][0:2], psi1[0:LX][0:LY][0:2], psi2[0:LX][0:LY][0:2])
 {

#pragma acc parallel loop tile(32,2)
  //Sum over 0,1 directions.
  for(int x=0; x<LX; x++) { //in Dr Howarth's phi4 code, there was no '{' here -TW
    xp1 = (x+1)%LX;
    xm1 = (x-1+LX)%LX;
    for(int y=0; y<LY; y++) {
      yp1 = (y+1)%LY;
...
#pragma acc update self( psi2[0:LX][0:LY][0:2])
 } //acc pragma
}
...
#pragma acc data present(gauge[0:LX][0:LY][0:2]) copyout(psi1[0:LX][0:LY][0:2], psi2[0:LX][0:LY][0:2]) create(temp[0:LX][0:LY][0:2])
  {
...
#pragma acc update device(temp[0:LX][0:LY][0:2])
