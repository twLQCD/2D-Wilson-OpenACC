...
typedef struct{
  ...
  //HMC
  int nstep = 25; //10 makes this run
  double tau = 1.0;
  int iterHMC = 100; //10 makes this run
  int therm = 25; //2 makes this run
  int skip = 1; //1 makes this run
  int checkpoint = 10; //5 makes this run
...
void constructName(string &name, param_t p) {
  ...
  //cout << "in name construction " << endl;
}
...
void gaussStart(Complex gauge[LX][LY][2], param_t p){
  ...
  for(int x=0; x<LX; x++)
    for(int y=0; y<LY; y++){
      gauge[x][y][0] = polar(1.0,sqrt(1.0/p.beta)*(double(rand())/ (RAND_MAX)));
      gauge[x][y][1] = polar(1.0,sqrt(1.0/p.beta)*(double(rand())/ (RAND_MAX)));
    }
  return;
}  
...
void gaussComplex_F(Complex eta[LX][LY][2], param_t p) {
  ...
  for(int x=0; x<LX; x++) {
    for(int y=0; y<LY; y++) {
      for(int s=0; s<2; s++) {
	r1 = sqrt(-2.0*log((double(rand()))/ (RAND_MAX)));
	theta1 = TWO_PI*(double(rand())/ (RAND_MAX));
	r2 = sqrt(-2.0*log((double(rand())/ (RAND_MAX))));
	theta2 = TWO_PI*(double(rand())/ (RAND_MAX));
