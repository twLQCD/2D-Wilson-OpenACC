...
int Ainvpsi(Complex x[LX][LY][2], Complex b[LX][LY][2], Complex x0[LX][LY][2],
  ...
  double timeinv = -((double)clock());
 
  double tmptime = 0.0;

#pragma acc data present(gauge[0:LX][0:LY][0:2]) copyout(b[0:LX][0:LY][0:2]) create(p[0:LX][0:LY][0:2])
  {
...
#pragma acc update device(p[0:LX][0:LY][0:2])
  ...
  for (k=0; k<param.maxIterCG; k++) {
    ...
    // Compute Ap.
    double timemvps1 = -((double)clock());
    DdagDpsi(Ap, p, gauge, param);
    double timeendmvps1 = timemvps1 + clock();
    tmptime += timeendmvps1;
  ...
  double timemvps2 = -((double)clock());
  DdagDpsi(tmp, x, gauge, param);
  double timeendmvps2 = timemvps2 + clock();
  axpy(-1.0, tmp, b, res);

  double timeendinv = timeinv + clock();
  name = "./data/data/invtime";
  name += ".dat";
  sprintf(fname, "%s", name.c_str());
  fp = fopen(fname, "a");
  fprintf(fp, "%.16e \n", timeendinv/CLOCKS_PER_SEC);
  fclose(fp);

  name = "./data/data/mvpstime";
  name += ".dat";
  sprintf(fname, "%s", name.c_str());
  fp = fopen(fname, "a");
  fprintf(fp, "%.16e \n", (tmptime + timeendmvps2)/CLOCKS_PER_SEC);
  fclose(fp);
...
void forceD(double fD[LX][LY][2], Complex gauge[LX][LY][2], Complex phi[LX][LY][2],
  ...
  double timefD = -((double)clock());

#pragma acc data present(gauge[0:LX][0:LY][0:2], phi[0:LX][0:LY][0:2],  guess[0:LX][0:LY][0:2])
  {
  ...
  if(p.dynamic == true) {
   ...
   double timeendfD = timefD + clock();
   name = "./data/data/fDtime";
   constructName(name, p);
   name += ".dat";
   sprintf(fname, "%s", name.c_str());
   fp = fopen(fname, "a");
   fprintf(fp, "%.16e \n", timeendfD/CLOCKS_PER_SEC);
   fclose(fp);
  } //acc pragma
}
